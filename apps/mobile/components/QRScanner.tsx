...
export default function QRScanner({ onScanComplete }: QRScannerProps) {
  ...
  const { ip, port } = useServerStore(useShallow((s) => ({ ip: s.ip, port: s.settings.port })));
  ...
  useEffect(() => {
    ...
    const onConnectError = (err: Error) => {
      ...
      setSocketError('Could not connect to relay server. Pairing may not work.');
    };
  ...
  const handleBarcodeScanned = useCallback(
    ({ data }: { data: string }) => {
      ...
      socket.current?.emit('private_message', {
        to: data,
        msg: JSON.stringify({ ip: serverInfoRef.current.ip, port: serverInfoRef.current.port }),
      });
