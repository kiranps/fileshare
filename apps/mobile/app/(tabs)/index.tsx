...
export default function ServerScreen() {
    ...
    return showQR ? (
        <QRScanner onScanComplete={() => setShowQR(false)} />
    ) : (
                    ...
                    >
                        <View className="mb-4 flex-row items-center justify-between">
                            <View className="flex-row items-center gap-3">
                                <View
                                    className={`rounded-full p-3 ${isRunning ? "bg-primary" : "bg-muted"}`}
                                >
                                    <ServerIcon
                                        size={28}
                                        color={isRunning ? "white" : "gray"}
                                    />
                                </View>
                                <View>
                                    <Text
                ...
                <View className="px-6">
                    ...
                    <View className="rounded-xl border border-border/50 bg-muted/30 p-4">
                        <Text className="text-sm leading-relaxed text-muted-foreground">
                            1. Make sure your phone and computer are on the same
                            Wi‑Fi network.
                            {"\n\n"}
                            2. Tap “Start sharing” below.
                            {"\n\n"}
                            3. On your computer, open the link shown above in a
                            browser or file explorer to browse and download
                            files.
                        </Text>
                        {isRunning && (
                            <TouchableOpacity
