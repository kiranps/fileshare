...
export function parseWebDAVPropfindResponse(xml: string): WebDAVEntry[] {
  ...
  return responses.map((node) => {
    ...
    const contentType = propNode?.getElementsByTagNameNS("*", "getcontenttype")[0]
      ?.textContent ?? undefined;
    const contentLength = propNode?.getElementsByTagNameNS("*", "getcontentlength")[0]
      ?.textContent;
    ...
    const lastModifiedText = propNode?.getElementsByTagNameNS("*", "getlastmodified")[0]
      ?.textContent;
